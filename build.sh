...
# Generate archive-keyring quickpkg
cat <<EOF > "./packages/${PROJECT_NAME}-archive-keyring.cfg"
Version = ${KEY_VERSION}
Description = OpenPGP archive certificates of ${PROJECT_NAME}
...
content =
 ...
 |
 |   Autogenerated keyring package.
 |
 |  -- ${KEY_AUTHOR}  $(date -u -R)

[/etc/apt/sources.list.d/${PROJECT_NAME}.sources]
content =
 | Types: deb
 | URIs: ${PUBLISH_URL}
 | Suites: ${PROJECT_REL}
 | Components: main
 | Signed-By: /usr/share/keyrings/${PROJECT_NAME}.pgp

[/usr/share/keyrings/${PROJECT_NAME}.pgp]
content_hex =
EOF
gpg --export-options export-minimal --export "${KEY_AUTHOR}" | xxd -p | sed -e 's/^/ | /' >> "./packages/${PROJECT_NAME}-archive-keyring.cfg"
